...
import java.util.*;
public record BinanceConfig(String baseUrl, List<String> symbols, List<String> series, List<String> tickers, List<String> protocols, String interval, int limit, int concurrency, Retry retry, RateLimit rateLimit, Paths paths) {
    ...
    public static BinanceConfig fromYaml(Path p) throws IOException {
        Yaml yaml=new Yaml();
        ...
        List<String> symbols=(List<String>)m.getOrDefault("symbols", m.getOrDefault("series", m.getOrDefault("tickers", m.getOrDefault("protocols", List.of("binance")))));
        // normalize
        ...
        Paths paths=new Paths((String)pa.getOrDefault("bronze","data/bronze/binance"), (String)pa.getOrDefault("silver","data/silver/binance"), (String)pa.getOrDefault("catalog","catalog/glue.json"));
        return new BinanceConfig(baseUrl, symbols, series, tickers, protocols, interval, limit, concurrency, retry, rateLimit, paths);
    }
    public static BinanceConfig defaults(){ return new BinanceConfig("https://api.binance.com", List.of("binance"), List.of("binance"), List.of("binance"), List.of("binance"), "1d",30,2,new Retry(3,800,8000), new RateLimit(2,4), new Paths("data/bronze/binance","data/silver/binance","catalog/glue.json")); }
    // convenience
    public List<String> effectiveKeys(){
        if(!series.isEmpty() && !series.get(0).equals("binance")) return series;
        if(!tickers.isEmpty() && !tickers.get(0).equals("binance")) return tickers;
        if(!protocols.isEmpty() && !protocols.get(0).equals("binance")) return protocols;
        return symbols;
    }
}
