...
import java.util.*;
public record CboeConfig(String baseUrl, List<String> symbols, List<String> series, List<String> tickers, List<String> protocols, String interval, int limit, int concurrency, Retry retry, RateLimit rateLimit, Paths paths) {
    ...
    public static CboeConfig fromYaml(Path p) throws IOException {
        Yaml yaml=new Yaml();
        ...
        List<String> symbols=(List<String>)m.getOrDefault("symbols", m.getOrDefault("series", m.getOrDefault("tickers", m.getOrDefault("protocols", List.of("cboe")))));
        // normalize
        ...
        Paths paths=new Paths((String)pa.getOrDefault("bronze","data/bronze/cboe"), (String)pa.getOrDefault("silver","data/silver/cboe"), (String)pa.getOrDefault("catalog","catalog/glue.json"));
        return new CboeConfig(baseUrl, symbols, series, tickers, protocols, interval, limit, concurrency, retry, rateLimit, paths);
    }
    public static CboeConfig defaults(){ return new CboeConfig("https://cdn.cboe.com", List.of("cboe"), List.of("cboe"), List.of("cboe"), List.of("cboe"), "1d",30,2,new Retry(3,800,8000), new RateLimit(2,4), new Paths("data/bronze/cboe","data/silver/cboe","catalog/glue.json")); }
    // convenience
    public List<String> effectiveKeys(){
        if(!series.isEmpty() && !series.get(0).equals("cboe")) return series;
        if(!tickers.isEmpty() && !tickers.get(0).equals("cboe")) return tickers;
        if(!protocols.isEmpty() && !protocols.get(0).equals("cboe")) return protocols;
        return symbols;
    }
}
