...
import java.util.*;
public record EastmoneyConfig(String baseUrl, List<String> symbols, List<String> series, List<String> tickers, List<String> protocols, String interval, int limit, int concurrency, Retry retry, RateLimit rateLimit, Paths paths) {
    ...
    public static EastmoneyConfig fromYaml(Path p) throws IOException {
        Yaml yaml=new Yaml();
        ...
        List<String> symbols=(List<String>)m.getOrDefault("symbols", m.getOrDefault("series", m.getOrDefault("tickers", m.getOrDefault("protocols", List.of("eastmoney")))));
        // normalize
        ...
        Paths paths=new Paths((String)pa.getOrDefault("bronze","data/bronze/eastmoney"), (String)pa.getOrDefault("silver","data/silver/eastmoney"), (String)pa.getOrDefault("catalog","catalog/glue.json"));
        return new EastmoneyConfig(baseUrl, symbols, series, tickers, protocols, interval, limit, concurrency, retry, rateLimit, paths);
    }
    public static EastmoneyConfig defaults(){ return new EastmoneyConfig("https://push2.eastmoney.com", List.of("eastmoney"), List.of("eastmoney"), List.of("eastmoney"), List.of("eastmoney"), "1d",30,2,new Retry(3,800,8000), new RateLimit(2,4), new Paths("data/bronze/eastmoney","data/silver/eastmoney","catalog/glue.json")); }
    // convenience
