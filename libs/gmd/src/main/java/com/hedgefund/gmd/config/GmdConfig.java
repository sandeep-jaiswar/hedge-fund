...
import java.util.*;
public record GmdConfig(String baseUrl, List<String> symbols, List<String> series, List<String> tickers, List<String> protocols, String interval, int limit, int concurrency, Retry retry, RateLimit rateLimit, Paths paths) {
    ...
    public static GmdConfig fromYaml(Path p) throws IOException {
        Yaml yaml=new Yaml();
        ...
        List<String> symbols=(List<String>)m.getOrDefault("symbols", m.getOrDefault("series", m.getOrDefault("tickers", m.getOrDefault("protocols", List.of("gmd")))));
        // normalize
        ...
        Paths paths=new Paths((String)pa.getOrDefault("bronze","data/bronze/gmd"), (String)pa.getOrDefault("silver","data/silver/gmd"), (String)pa.getOrDefault("catalog","catalog/glue.json"));
        return new GmdConfig(baseUrl, symbols, series, tickers, protocols, interval, limit, concurrency, retry, rateLimit, paths);
    }
    public static GmdConfig defaults(){ return new GmdConfig("https://raw.githubusercontent.com", List.of("gmd"), List.of("gmd"), List.of("gmd"), List.of("gmd"), "1d",30,2,new Retry(3,800,8000), new RateLimit(2,4), new Paths("data/bronze/gmd","data/silver/gmd","catalog/glue.json")); }
    // convenience
    public List<String> effectiveKeys(){
        if(!series.isEmpty() && !series.get(0).equals("gmd")) return series;
        if(!tickers.isEmpty() && !tickers.get(0).equals("gmd")) return tickers;
        if(!protocols.isEmpty() && !protocols.get(0).equals("gmd")) return protocols;
        return symbols;
    }
}
