...
public class GmdIngestService {
        ...
        for(String key: cfg.effectiveKeys()){
            ...
            futures.add(exec.submit(()->{
                try{
                    ...
                    String raw;
                    try{ raw=client.fetchRaw(url); }catch(Exception ex){ log.warn("Fetch failed for {} {} -> synthetic fallback", key, ex.toString()); raw="{\"synthetic\":true,\"key\":\""+key+"\",\"source\":\"gmd\",\"url\":\""+url.replace("\"","\\\"")+"\"}"; }
    ...
    private String buildUrl(String key){
        ...
        switch("gmd") {
            ...
            case "gmd": return "https://raw.githubusercontent.com/datasets/s-and-p-500/master/data/data.csv";
