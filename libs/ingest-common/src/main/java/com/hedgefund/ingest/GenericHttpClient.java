...
/** Shared HTTP client with throttling + retry — replaces 20 identical BlsClient copies. */
public class GenericHttpClient {
    ...
    private static final HttpClient SHARED = HttpClient.newBuilder()
            ...
            .build();
    private final GenericIngestConfig cfg;
    private final ObjectMapper om;
    private long lastRequestAt = 0;

    public GenericHttpClient(GenericIngestConfig cfg) {
        this.cfg = cfg;
        ...
        while (true) {
            ...
            try {
                ...
                HttpRequest.Builder b = HttpRequest.newBuilder(URI.create(url))
                        ...
                        .GET()
                        .header("User-Agent", "Mozilla/5.0 HedgeFund/1.0")
                        .header("Accept", "*/*");
                if (url.contains("sec.gov")) b.header("Accept-Encoding", "gzip");
                HttpResponse<String> resp = SHARED.send(b.build(), HttpResponse.BodyHandlers.ofString());
                if (resp.statusCode() == 429 || resp.statusCode() >= 500)
                    throw new java.io.IOException("HTTP " + resp.statusCode() + " " + resp.body().substring(0, Math.min(300, resp.body().length())));
                if (resp.statusCode() != 200)
                    throw new java.io.IOException("HTTP " + resp.statusCode() + " " + resp.body().substring(0, Math.min(400, resp.body().length())));
                return resp.body();
            } catch (Exception e) {
    ...
    public JsonNode fetchJson(String url) throws Exception {
        return om.readTree(fetchRaw(url));
    }
}
