...
public class InvestingIngestService {
        ...
        for(String key: cfg.effectiveKeys()){
            ...
            futures.add(exec.submit(()->{
                try{
                    ...
                    String raw;
                    try{ raw=client.fetchRaw(url); }catch(Exception ex){ log.warn("Fetch failed for {} {} -> synthetic fallback", key, ex.toString()); raw="{\"synthetic\":true,\"key\":\""+key+"\",\"source\":\"investing\",\"url\":\""+url.replace("\"","\\\"")+"\"}"; }
    ...
    private String buildUrl(String key){
        ...
        switch("investing") {
            ...
            case "investing": return "https://query1.finance.yahoo.com/v8/finance/chart/SPY?interval=1d&range=1mo";
