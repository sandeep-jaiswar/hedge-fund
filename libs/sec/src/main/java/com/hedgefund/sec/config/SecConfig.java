...
import java.util.*;
public record SecConfig(String baseUrl, List<String> symbols, List<String> series, List<String> tickers, List<String> protocols, String interval, int limit, int concurrency, Retry retry, RateLimit rateLimit, Paths paths) {
    ...
    public static SecConfig fromYaml(Path p) throws IOException {
        Yaml yaml=new Yaml();
        ...
        List<String> symbols=(List<String>)m.getOrDefault("symbols", m.getOrDefault("series", m.getOrDefault("tickers", m.getOrDefault("protocols", List.of("sec")))));
        // normalize
        ...
        Paths paths=new Paths((String)pa.getOrDefault("bronze","data/bronze/sec"), (String)pa.getOrDefault("silver","data/silver/sec"), (String)pa.getOrDefault("catalog","catalog/glue.json"));
        return new SecConfig(baseUrl, symbols, series, tickers, protocols, interval, limit, concurrency, retry, rateLimit, paths);
    }
    public static SecConfig defaults(){ return new SecConfig("https://www.sec.gov", List.of("sec"), List.of("sec"), List.of("sec"), List.of("sec"), "1d",30,2,new Retry(3,800,8000), new RateLimit(2,4), new Paths("data/bronze/sec","data/silver/sec","catalog/glue.json")); }
    // convenience
    public List<String> effectiveKeys(){
        if(!series.isEmpty() && !series.get(0).equals("sec")) return series;
        if(!tickers.isEmpty() && !tickers.get(0).equals("sec")) return tickers;
        if(!protocols.isEmpty() && !protocols.get(0).equals("sec")) return protocols;
        return symbols;
    }
}
