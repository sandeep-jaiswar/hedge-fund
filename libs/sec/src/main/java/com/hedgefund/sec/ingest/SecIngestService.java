        ...
        for(String key: cfg.effectiveKeys()){
            ...
            futures.add(exec.submit(()->{
                try{
                    ...
                    String raw;
                    try{ raw=client.fetchRaw(url); }catch(Exception ex){ log.warn("Fetch failed for {} {} -> synthetic fallback", key, ex.toString()); raw="{\"synthetic\":true,\"key\":\""+key+"\",\"source\":\"sec\",\"url\":\""+url.replace("\"","\\\"")+"\"}"; }
