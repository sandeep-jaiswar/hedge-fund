...
import java.util.*;
public record SinaConfig(String baseUrl, List<String> symbols, List<String> series, List<String> tickers, List<String> protocols, String interval, int limit, int concurrency, Retry retry, RateLimit rateLimit, Paths paths) {
    ...
    public static SinaConfig fromYaml(Path p) throws IOException {
        Yaml yaml=new Yaml();
        ...
        List<String> symbols=(List<String>)m.getOrDefault("symbols", m.getOrDefault("series", m.getOrDefault("tickers", m.getOrDefault("protocols", List.of("sina")))));
        // normalize
        ...
        Paths paths=new Paths((String)pa.getOrDefault("bronze","data/bronze/sina"), (String)pa.getOrDefault("silver","data/silver/sina"), (String)pa.getOrDefault("catalog","catalog/glue.json"));
        return new SinaConfig(baseUrl, symbols, series, tickers, protocols, interval, limit, concurrency, retry, rateLimit, paths);
    }
    public static SinaConfig defaults(){ return new SinaConfig("https://hq.sinajs.cn", List.of("sina"), List.of("sina"), List.of("sina"), List.of("sina"), "1d",30,2,new Retry(3,800,8000), new RateLimit(2,4), new Paths("data/bronze/sina","data/silver/sina","catalog/glue.json")); }
    // convenience
