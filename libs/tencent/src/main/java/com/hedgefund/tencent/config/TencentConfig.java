...
import java.util.*;
public record TencentConfig(String baseUrl, List<String> symbols, List<String> series, List<String> tickers, List<String> protocols, String interval, int limit, int concurrency, Retry retry, RateLimit rateLimit, Paths paths) {
    ...
    public static TencentConfig fromYaml(Path p) throws IOException {
        Yaml yaml=new Yaml();
        ...
        List<String> symbols=(List<String>)m.getOrDefault("symbols", m.getOrDefault("series", m.getOrDefault("tickers", m.getOrDefault("protocols", List.of("tencent")))));
        // normalize
        ...
        Paths paths=new Paths((String)pa.getOrDefault("bronze","data/bronze/tencent"), (String)pa.getOrDefault("silver","data/silver/tencent"), (String)pa.getOrDefault("catalog","catalog/glue.json"));
        return new TencentConfig(baseUrl, symbols, series, tickers, protocols, interval, limit, concurrency, retry, rateLimit, paths);
    }
    public static TencentConfig defaults(){ return new TencentConfig("https://qt.gtimg.cn", List.of("tencent"), List.of("tencent"), List.of("tencent"), List.of("tencent"), "1d",30,2,new Retry(3,800,8000), new RateLimit(2,4), new Paths("data/bronze/tencent","data/silver/tencent","catalog/glue.json")); }
    // convenience
