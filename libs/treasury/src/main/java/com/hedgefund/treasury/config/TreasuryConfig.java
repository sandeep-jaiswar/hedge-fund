...
import java.util.*;
public record TreasuryConfig(String baseUrl, List<String> symbols, List<String> series, List<String> tickers, List<String> protocols, String interval, int limit, int concurrency, Retry retry, RateLimit rateLimit, Paths paths) {
    ...
    public static TreasuryConfig fromYaml(Path p) throws IOException {
        Yaml yaml=new Yaml();
        ...
        List<String> symbols=(List<String>)m.getOrDefault("symbols", m.getOrDefault("series", m.getOrDefault("tickers", m.getOrDefault("protocols", List.of("treasury")))));
        // normalize
        ...
        Paths paths=new Paths((String)pa.getOrDefault("bronze","data/bronze/treasury"), (String)pa.getOrDefault("silver","data/silver/treasury"), (String)pa.getOrDefault("catalog","catalog/glue.json"));
        return new TreasuryConfig(baseUrl, symbols, series, tickers, protocols, interval, limit, concurrency, retry, rateLimit, paths);
    }
    public static TreasuryConfig defaults(){ return new TreasuryConfig("https://home.treasury.gov", List.of("treasury"), List.of("treasury"), List.of("treasury"), List.of("treasury"), "1d",30,2,new Retry(3,800,8000), new RateLimit(2,4), new Paths("data/bronze/treasury","data/silver/treasury","catalog/glue.json")); }
    // convenience
    public List<String> effectiveKeys(){
        if(!series.isEmpty() && !series.get(0).equals("treasury")) return series;
        if(!tickers.isEmpty() && !tickers.get(0).equals("treasury")) return tickers;
        if(!protocols.isEmpty() && !protocols.get(0).equals("treasury")) return protocols;
        return symbols;
    }
}
